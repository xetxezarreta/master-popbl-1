...
def health_check():
    # print("Health check on payment")
    return "OK", 200


# Payment Routes
@app.route('/payment/perform_deposit', methods=['POST'])
def perform_deposit():
    if request.headers['Content-Type'] != 'application/json':
        abort(UnsupportedMediaType.code)
    content = request.json

    if "quantity" in content and "clientId" in content:
        logic.add_payment(content['quantity'], content['clientId'])
        return {"success": True}
    else:
        return {"success": False}


@app.route('/payment/<int:client_id>', methods=['GET'])
def get_payment(client_id):
    return logic.get_client_payment(client_id)
